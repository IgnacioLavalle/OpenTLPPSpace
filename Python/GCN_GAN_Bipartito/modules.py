...
class GCN_GAN(nn.Module):
    ...
    def forward(self, real, fake):
        '''
        :param real: ground-truth (tensor 1D o 2D)
        :param fake: predicción (tensor 1D o 2D)
        :return: salida del discriminador para real y fake
        '''
        # Asegurar que sea 2D (batch_size=1 si es vector)
        if real.dim() == 1:
        ...
        for l in range(self.num_disc_layers - 1):
            real_input = torch.relu(self.disc[l](real_input))
            real_input = self.disc_drop[l](real_input)
        real_output = self.disc[-1](real_input)

        # Procesamiento fake
        fake_input = fake
        for l in range(self.num_disc_layers - 1):
            fake_input = torch.relu(self.disc[l](fake_input))
            fake_input = self.disc_drop[l](fake_input)
        fake_output = self.disc[-1](fake_input)
