...
def main():
    ...
    for tau in range(start_test, num_snaps):
        ...
        precision_vals, recall_vals, prrc_thresholds = precision_recall_curve(true_labels, pred_scores)
        avg_prec = average_precision_score(true_labels, pred_scores)

        ##Queda medio desprolijo pero, la idea es: 
        #en el primer snapshot elijo el mejor threshold y luego genero el forecast

        if first_snapshot:
            print()
            f1_scores = 2*recall_vals*precision_vals/(recall_vals+precision_vals)
            print('Best threshold: ', prrc_thresholds[np.argmax(f1_scores)])
            best_th=np.max(f1_scores)
            print('Best F1-Score with given threshold: ', best_th)
            first_snapshot = False
            print()



        pred_labels = (pred_vals >= best_th).astype(int)
    ...
    if save_metrics:
        ...
        filename = f"gcngan_bth_fc_curve_metrics_from_year{start_test}.json"
